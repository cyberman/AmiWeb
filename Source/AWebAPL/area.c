...
static void Setcoords(struct Area *area,USHORT shape,UBYTE *coords)
{  long nrcoords,i,n;
   UBYTE *p;
   BOOL ok=FALSE;
   ...
   {  if(area->coords)
      ...
      if(nrcoords)
      {  if(area->coords=PALLOCTYPE(long,nrcoords,MEMF_CLEAR,area->pool))
...
{  long result;
   ...
   result=Amethodas(AOTP_LINK,area,AOM_SET,ams->tags);
...
{  long result;
   struct TagItem *tag,*tstate=ams->tags;
   result=AmethodasA(AOTP_LINK,area,ams);
   while(tag=NextTagItem(&tstate))
   {  switch(tag->ti_Tag)
      {  case AOARA_Shape:
            ...
            break;
         case AOARA_Tooltip:
            if(area->text && area->textpos>0 && area->textlen>0 && !area->tooltip)
            {  area->tooltip=Dupstr(area->text->buffer+area->textpos,area->textlen);
            }
...
static long Dispatch(struct Area *area,struct Amessage *amsg)
{  long result=0;
   switch(amsg->method)
   {  case AOM_NEW:
...
*/
      ...
      case AOM_DEINSTALL:
         break;
      default:
         result=AmethodasA(AOTP_LINK,area,amsg);
   }
   return result;
}
...
{  if(!Amethod(NULL,AOM_INSTALL,AOTP_AREA,Dispatch)) return FALSE;
   return TRUE;
}
...
{  return Runjavascript(area->cframe,area->onclick,&area->jobject);
}
