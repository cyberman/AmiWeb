...
{  /* Always return NEWLINE if no soft break.
   ...
   {  aml->amlr->endx=aml->startx;
      if(aml->flags&AMLF_FIRST) aml->amlr->toph=br->aoh;
...
{  long result;
   ...
   result=Amethodas(AOTP_ELEMENT,br,AOM_SET,ams->tags);
...
{  long result;
   struct TagItem *tag,*tstate=ams->tags;
   result=AmethodasA(AOTP_ELEMENT,br,ams);
   while(tag=NextTagItem(&tstate))
   {  switch(tag->ti_Tag)
      {  case AOBJ_Clipdrag:
            PUTATTR(tag,TRUE);
...
{  long result=0;
   ...
   if((br->aoy<=y && br->aoy+br->aoh>y && br->aox<=x) || br->aoy>y)
   {  result=AMDR_HIT;
      if(amd->amdr)
      {  amd->amdr->object=br;
      }
...
static long Dispatch(struct Break *br,struct Amessage *amsg)
{  long result=0;
   switch(amsg->method)
   {  case AOM_NEW:
      ...
      case AOM_DEINSTALL:
         break;
      default:
         result=AmethodasA(AOTP_ELEMENT,br,amsg);
   }
   return result;
}
...
{  if(!Amethod(NULL,AOM_INSTALL,AOTP_BREAK,Dispatch)) return FALSE;
   return TRUE;
}
