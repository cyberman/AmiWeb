   ...
   {  switch(tag->ti_Tag)
      {  case AOBJ_Left:
         ...
         case AOELT_Link:
            ...
            if(elt->link) Aaddchild(elt->link,elt,0);
            break;
...
static long Dispatch(struct Element *elt,struct Amessage *amsg)
{  long result=0;
   switch(amsg->method)
   {  case AOM_SET:
...
{  InitRastPort(&eltrp);
   mrp=&eltrp;
   if(!Amethod(NULL,AOM_INSTALL,AOTP_ELEMENT,Dispatch)) return FALSE;
   return TRUE;
}
