...
static USHORT Resizehit(struct Frameset *frs,long x,long y,struct Child *ch1,struct Child *ch2)
{  USHORT ptrtype=APTR_DEFAULT;
...
{  struct Child *ch1,*ch2;
   ...
   {  if(frs->flags&FRSF_ROWS)
      ...
      {  Rendersize(frs,NULL);
         ...
         frs->sizechild=NULL;
         Arender(frs,NULL,0,0,AMRMAX,AMRMAX,AMRF_CLEAR,NULL);
      }
   }
}
...
{  struct Child *ch;
   ...
   long relprc;         /* Percentage left for all relative sized combined */
...
{  struct Coords *coo;
   ...
   {  if((coo=Clipcoords(frs->frame,amr->coords)) && coo->rp)
      {  SetAPen(coo->rp,
         ...
         x1=MAX(amr->minx,frs->aox)+coo->dx;
         y1=MAX(amr->miny,frs->aoy)+coo->dy;
         x2=MIN(amr->maxx,frs->aox+frs->aow-1)+coo->dx;
         y2=MIN(amr->maxy,frs->aoy+frs->aoh-1)+coo->dy;
         if(x1<=x2 && y1<=y2)
...
{  struct TagItem *tag,*tstate=ams->tags;
   ...
   {  switch(tag->ti_Tag)
      {  case AOBJ_Left:
         ...
         case AOBJ_Window:
            ...
            {  Asetattrs(frs->win,AOWIN_Goinactive,frs,TAG_END);
            }
   ...
   {  for(ch=frs->childs.first;ch->next;ch=ch->next)
      {  if(ch->object)
         {  Asetattrs(ch->object,
               setframe?AOBJ_Frame:TAG_IGNORE,frs->frame,
               setwin?AOBJ_Window:TAG_IGNORE,frs->win,
               setwhis?AOBJ_Winhis:TAG_IGNORE,frs->whis,
...
{  struct Child *ch;
   ...
   {  if(!frs->subleft)
      ...
      if(frs->subchild)
      {  AmethodA(frs->subchild->object,ama);
         frs->subleft--;
      }
      else
...
{  struct Child *ch;
   while(ch=REMHEAD(&frs->childs))
   {  if(ch->object) Adisposeobject(ch->object);
      FREE(ch);
   }
   if(frs->subcols) FREE(frs->subcols);
   if(frs->win) Asetattrs(frs->win,AOWIN_Goinactive,frs,TAG_END);
   Amethodas(AOTP_OBJECT,frs,AOM_DISPOSE);
}

static long Dispatch(struct Frameset *frs,struct Amessage *amsg)
{  long result=0;
   switch(amsg->method)
   {  case AOM_NEW:
...
{  if(!Amethod(NULL,AOM_INSTALL,AOTP_FRAMESET,Dispatch)) return FALSE;
   return TRUE;
}
