...
static ULONG __saveds __asm Renderhook(register __a0 struct Hook *hook,
...
{  ULONG result=LBCB_UNKNOWN;
   ...
   {  if(hook->h_Data==(APTR)LIST_GROUPS)
      ...
      slack=(rect.MaxY-rect.MinY)-(te.MaxY-te.MinY);
      x=rect.MinX-te.MinX+1;
      y=rect.MinY-te.MinY+(slack+1)/2;
      Move(rp,x,y);
...
{  short imgsize;
   ...
   grenderhook.h_Entry=(HOOKFUNC)Renderhook;
   grenderhook.h_Data=(APTR)LIST_GROUPS;
   lrenderhook.h_Entry=(HOOKFUNC)Renderhook;
...
__asm __saveds void Hotmgrtask(register __a0 struct Hotwindow *how)
{  struct Taskmsg *hm;
   ULONG done=FALSE;
   ULONG getmask;
