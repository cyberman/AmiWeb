   ...
   {  switch(tag->ti_Tag)
      {  case AOBJ_Pool:
         ...
         case AOMAP_Extcopy:
            ...
            if(newwin) Asetattrs(map->extcopy,AOBJ_Window,newwin,TAG_END);
            break;
         case AOMAP_Extname:
...
static long Dispatch(struct Map *map,struct Amessage *amsg)
{  long result=0;
   switch(amsg->method)
   {  case AOM_NEW:
...
{  if(!Amethod(NULL,AOM_INSTALL,AOTP_MAP,Dispatch)) return FALSE;
   return TRUE;
}
