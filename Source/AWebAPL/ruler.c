...
{  if(rul->flags&RULF_PIXELS)
   {  rul->aow=MIN(rul->width,aml->width-aml->startx);
   }
   else
   {  rul->aow=(aml->width-aml->startx)*rul->width/100;
   }
   rul->aoh=rul->size+6;
   rul->aox=aml->startx;
   if(aml->amlr)
   {  aml->amlr->result=AMLR_NEWLINE;
      aml->amlr->endx=rul->aox+rul->aow;
      aml->amlr->toph=rul->aoh;
   }
   return 0;
}
...
{  long result;
   ...
   result=Amethodas(AOTP_ELEMENT,rul,AOM_SET,ams->tags);
...
{  struct Ruler *rul;
   ...
   {  rul->width=100;
      ...
      Asetattrs(rul,AOELT_Halign,HALIGN_CENTER,TAG_END);
      Setruler(rul,ams);
   }
   return rul;
}

static long Dispatch(struct Ruler *rul,struct Amessage *amsg)
{  long result=0;
   switch(amsg->method)
   {  case AOM_NEW:
      ...
      case AOM_DEINSTALL:
         break;
      default:
         result=AmethodasA(AOTP_ELEMENT,rul,amsg);
   }
   return result;
}
...
{  if(!Amethod(NULL,AOM_INSTALL,AOTP_RULER,Dispatch)) return FALSE;
   return TRUE;
}
