...
static USHORT Icontype(struct Saveas *sas)
{  USHORT icon=FILEICON_NONE;
...
{  long result;
   ...
   result=Amethodas(AOTP_OBJECT,sas,AOM_SET,ams->tags);
...
{  long result;
   struct TagItem *tag,*tstate=ams->tags;
   result=AmethodasA(AOTP_OBJECT,sas,ams);
   while(tag=NextTagItem(&tstate))
   {  switch(tag->ti_Tag)
      {  case AOSDV_Source:
      ...
      {  /* Check content type - never prompt to save text/html */
         ...
         {  if(!(name=Savepathsrc(sas->source)))
            ...
            sas->req=Anewobject(AOTP_FILEREQ,
               ...
               sas->unktype?AOFRQ_Unkurl:TAG_IGNORE,unkurl,
               sas->unktype?AOFRQ_Unktype:TAG_IGNORE,sas->unktype,
               sas->unktype?AOFRQ_Unkext:TAG_IGNORE,unkext?unkext:(UBYTE *)"(none)",
               AOBJ_Target,sas,
               TAG_END);
            if(name) FREE(name);
...
static long Dispatch(struct Saveas *sas,struct Amessage *amsg)
{  long result=0;
   switch(amsg->method)
   {  case AOM_NEW:
...
{  if(!Amethod(NULL,AOM_INSTALL,AOTP_SAVEAS,Dispatch)) return FALSE;
   return TRUE;
}
