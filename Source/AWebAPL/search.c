...
{  struct Amsearch ams={0};
   ...
   {  ams.method=AOM_SEARCHSET;
      ...
      ams.text=sr->buffer;
      ams.top=-1;
      AmethodA(sr->frame,&ams);  /* Clears our pos when frame scrolls */
      sr->pos=newpos;
...
{  struct Search *sr,*nextsr;
   ULONG result;
   BOOL done;
   for(sr=searchreqs.first;sr->next;sr=nextsr)
   {  nextsr=sr->next;
      done=FALSE;
      if(sr->nfwinobj)
...
{  REMOVE(sr);
   Aremchild(Aweb(),sr,AOREL_APP_USE_SCREEN);
   Unhighlight(sr);
   Closesearchreq(sr);
   Closenfwindow(sr);
   if(sr->title) FREE(sr->title);
...
{  struct Search *sr;
   ...
   {  ADDTAIL(&searchreqs,sr);
      sr->pos=-1;
      Setsearch(sr,ams);
      Aaddchild(Aweb(),sr,AOREL_APP_USE_SCREEN);
      Asetattrs(Aweb(),
         AOAPP_Processtype,AOTP_SEARCH,
         AOAPP_Processfun,Processsearch,
         TAG_END);
...
{  struct Search *sr;
   while((sr=searchreqs.first) && sr->next)
   {  Asetattrs(sr->frame,AOFRM_Search,FALSE,TAG_END);
   }
   if(laststring) FREE(laststring);
}

static long Dispatch(struct Search *sr,struct Amessage *amsg)
{  long result=0;
   switch(amsg->method)
   {  case AOM_NEW:
...
{  NEWLIST(&searchreqs);
   if(!Amethod(NULL,AOM_INSTALL,AOTP_SEARCH,Dispatch)) return FALSE;
   return TRUE;
}
