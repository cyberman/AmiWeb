...
{  long result;
   ...
   result=Amethodas(AOTP_OBJECT,sns,AOM_SET,ams->tags);
...
{  long result;
   struct TagItem *tag,*tstate=ams->tags;
   result=AmethodasA(AOTP_OBJECT,sns,ams);
   while(tag=NextTagItem(&tstate))
   {  switch(tag->ti_Tag)
      {  case AOSDV_Source:
...
static long Dispatch(struct Sndsource *sns,struct Amessage *amsg)
{  long result=0;
   switch(amsg->method)
   {  case AOM_NEW:
      ...
      case AOM_DEINSTALL:
         ...
         break;
      default:
         result=AmethodasA(AOTP_OBJECT,sns,amsg);
   }
   return result;
}
...
{  if(!Amethod(NULL,AOM_INSTALL,AOTP_SOUNDSOURCE,Dispatch)) return FALSE;
   return TRUE;
}
