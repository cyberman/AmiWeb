...
{  struct Element elt;
   ...
   USHORT tabframe;           /* Outer border frame type */
   short bortop,borbottom,borleft,borright;  /* Borders for each side */
   USHORT rules;              /* Rules type */
   ...
   USHORT capstate;           /* "row state" for caption */
};
...
{  NODE(Coldef);
   ...
   USHORT swtype;             /* Suggested width type */
...
{  NODE(Tabrow);
   ...
   struct Colorinfo *bgcolor,*bordercolor,*borderdark,*borderlight;
                              /* Common colours */
   USHORT flags;
   USHORT state;              /* Used in incremental display, see below */
};
...
{  NODE(Tabcell);
   ...
   USHORT shtype;             /* Suggested height type (relative not supported) */
   USHORT flags;
...
static void Addcoldefs(struct Table *tab,struct TagItem *tstate,USHORT flags)
{  struct TagItem *tag;
   struct Coldef *cd;
   short span=1;
   short swidth=1;
   USHORT swtype=SWT_RELATIVE;
...
{  struct Tabrow *tr;
   LIST(Tabrow) *list;
   if(tab->flags&TABF_TFOOT) list=&tab->tfoot;
   else list=&tab->rows;
   ...
   if(tr=PALLOCSTRUCT(Tabrow,1,MEMF_PUBLIC|MEMF_CLEAR,tab->pool))
   {  NEWLIST(&tr->cells);
      ADDTAIL(list,tr);
      tr->rownr=nr;
   }
...
{  struct Tabrow *tr=Gettabrow(tab,rownr);
   ...
   short i;
   if(!tr) return NULL;
   if(tc=PALLOCSTRUCT(Tabcell,1,MEMF_PUBLIC|MEMF_CLEAR,tab->pool))
   {  if(!cellnr)
...
{  struct TagItem *tag;
   ...
   {  if(tab->flags&TABF_OPENROW)
      ...
      tab->caption=Anewobject(AOTP_BODY,
         AOBJ_Pool,tab->pool,
         AOBJ_Frame,tab->frame,
         AOBJ_Cframe,tab->cframe,
         AOBJ_Window,tab->win,
         AOBJ_Nobackground,BOOLVAL(tab->flags&TABF_NOBACKGROUND),
         AOBJ_Layoutparent,tab,
            ...
            {  switch(tag->ti_Tag)
               {  case AOTAB_Halign:
                  ...
                  case AOTAB_Valign:
                     ...
                     break;
                  case AOTAB_Rowspan:
                     tc->rowspan=MAX(0,(short)tag->ti_Data);
                     if(tc->rowspan==0)
                     {  tc->rowspan=1;
                     ...
                     break;
                  case AOTAB_Colspan:
                     tc->colspan=MAX(0,(short)tag->ti_Data);
                     if(tc->colspan==0)
                     {  tc->colspan=1;
         ...
         {  if(cd0->next) cd=cd0;
            ...
            {  if(r>0 || c>0) /* We've got (r==0,c==0) already */
               ...
               {  tcs->bordercolor=tc->bordercolor;
                  ...
                  if(tab->rules==TABRUL_GROUPS)
                  {  if(r==0 && (tr->flags&TABRF_FIRSTROW)) tcs->flags|=TABCF_BORTOP;
                     /* Set bottom border, will be removed when another row
                      * is added in the same group */
                     if(r==tc->rowspan-1) tcs->flags|=TABCF_BORBOTTOM;
                     if(c==0 && cd && (cd->flags&CDF_FIRSTCOL)) tcs->flags|=TABCF_BORLEFT;
...
static void Cutoffrows(struct Table *tab,LIST(Tabrow) *list)
{  struct Tabrow *tr;
...
{  struct Tabrow *tr;
   y=y-tab->aoy-tab->spacing/2;   /* Make it a relative row y */
   for(tr=tab->rows.first;tr->next;tr=tr->next)
   {  if(tr->y+tr->height>y)
   ...
   && (tab->flags&TABF_CAPBOTTOM))
   {  if(tab->capstate>TABRS_COMPLETE) tab->capstate=TABRS_COMPLETE;
      tab->aoh=tab->rows.last->y+tab->rows.last->height+tab->spacing;
   }
   if(tab->incrementaly>tab->aoh) tab->incrementaly=tab->aoh;
}
...
{  long x1,y1,x2,y2;
   ...
   {  if(prefs.docolors && ci1 && ci1->pen>=0) pen=ci1->pen;
      ...
      y2=y1+tr->height+tab->spacing+1;
      if(tc->flags&TABCF_BORTOP) y1+=tab->spacing/2;
      if(tc->flags&TABCF_BORBOTTOM) y2-=tab->spacing/2;
   ...
   {  if(prefs.docolors && ci1 && ci1->pen>=0) pen=ci1->pen;
      ...
      y1=tab->aoy+coo->dy+tr->y+tab->spacing/2-1;
      if(tc->flags&TABCF_BORLEFT) x1+=tab->spacing/2;
      if(tc->flags&TABCF_BORRIGHT) x2-=tab->spacing/2;
   ...
   {  if(prefs.docolors && ci2 && ci2->pen>=0) pen=ci2->pen;
      ...
      x1=tab->aox+coo->dx+hcol->x-1;
      x2=x1+hcol->width+tab->spacing+1;
      y1=tab->aoy+coo->dy+tr->y+(tab->spacing+1)/2+tr->height;
      if(tc->flags&TABCF_BORLEFT) x1+=tab->spacing/2;
      if(tc->flags&TABCF_BORRIGHT) x2-=tab->spacing/2;
      if(y1>=coo->miny && y1<=coo->maxy && x2>=coo->minx && x1<=coo->maxx)
      {  SetAPen(coo->rp,pen);
...
{  struct Tabrow *tr,*trs;
   struct Tabcell *tc;
   short i;
   long height,sheight,w,d,toth,lefth,desw,spacing,bloffset,winweb,minw,maxw,h;
   ...
   if((aml->flags&AMLF_RETRY) && (tab->eltflags&ELTF_ALIGNED))
   {  if(aml->amlr)
...
*/
    ...
    * tw = remaining table width to divide excluding spacing
    *
   ...
   if(tw<=minw-tab->nrcols*tab->spacing)
   {  for(i=0;i<tab->nrcols;i++)
      {  hcol=&tab->cols[i];
      ...
      {  /* There is enough room for the pixel sized columns.
         ...
         {  hcol=&tab->cols[i];
            ...
            {  hcol->width=MAX(hcol->minw,hcol->swidth);
               ...
               {  hcol->width=hcol->width*(desw-tab->nrcols*tab->spacing)/sizepix;
               }
               tw-=hcol->width;
            }
         }
 ...
 */
   ...
   {  Alayout(tab->caption,tab->aow-tab->borleft,aml->height,0,aml->text,0,NULL);
   }
   for(tr=tab->rows.first;tr->next;tr=tr->next)
            ...
            {  if(tc->rowspan==1)
               {  if(tc->valign==VALIGN_BASELINE)
                  ...
                  {  if(tc->shtype==SWT_PERCENT)
                     {  if(!(aml->flags&AMLF_INTABLE)) sheight=tc->sheight*aml->height/100;
                     }
                     else sheight=tc->sheight;
   ...
   {  Amove(tab->caption,tab->aox+tab->borleft,tab->aoh);
      tab->aoh+=Agetattr(tab->caption,AOBJ_Height);
   ...
   {  tab->aow+=tab->borright;
   }
   if((tab->flags&TABF_COMPLETE) && !(tab->flags&TABF_BBLAYEDOUT))
   {  tab->aoh+=tab->borbottom;
      tab->flags|=TABF_BBLAYEDOUT;
   }
   tab->flags|=TABF_LAYEDOUT;
   
   return (long)AmethodasA(AOTP_ELEMENT,tab,aml);
}
...
{  struct Tabrow *tr;
   struct Tabcell *tc;
   struct Ammove amm={0};
   if((tab->flags&TABF_INCREMENTAL) || !(tab->eltflags&ELTF_ALIGNED))
   {  amm.method=AOM_MOVE;
      amm.dx=ama->dx;
      amm.dy=ama->y;
      for(tr=tab->rows.first;tr->next;tr=tr->next)
...
{  struct Coords *coo,coords={0};
   ...
   {  if(!(coo=amr->coords))
      ...
      {  short save_bgcolor=coo->bgcolor;
         ...
         {  Arender(tab->caption,coo,amr->minx,amr->miny,amr->maxx,amr->maxy,
               0 /* amr->flags&AMRF_CLEARBG but amr->minx etc must be clipped */ ,amr->text);
         ...
         {  if(tr->state==TABRS_ALIGNED || (all && tr->state>TABRS_ALIGNED))
            {  for(tc=tr->cells.first;tc->next;tc=tc->next)
               {  if(tc->body)
                  {  x=tab->aox+tc->x;
                     y=tab->aoy+tc->y;
                     if(x<=amr->maxx && x+tc->width>amr->minx
                     && y<=amr->maxy && y+tc->height>amr->miny)
                     {  Arender(tc->body,coo,
                           MAX(amr->minx,x),
                           MAX(amr->miny,y),
                           MIN(amr->maxx,x+tc->width-1),
                           MIN(amr->maxy,y+tc->height-1),
                           amr->flags&AMRF_CLEARBG,amr->text);
                     }
                  }
                  if(tab->rules)
...
{  long result;
   ...
   void *winhis=NULL;
   long vspacing=0;
   result=Amethodas(AOTP_ELEMENT,tab,AOM_SET,ams->tags);
   ...
   {  struct TagItem tags[5];
      ...
      tags[0].ti_Data=(ULONG)tab->cframe;
...
{  long result;
   struct TagItem *tag,*tstate=ams->tags;
   result=AmethodasA(AOTP_ELEMENT,tab,ams);
   while(tag=NextTagItem(&tstate))
   {  switch(tag->ti_Tag)
      {  case AOTAB_Body:
...
{  struct Tabrow *tr;
   ...
   long x,y;
   BOOL ok=FALSE;
   x=amd->xco-amd->coords->dx-tab->aox;
   y=amd->yco-amd->coords->dy-tab->aoy;
   if(tab->caption && !(tab->flags&TABF_CAPBOTTOM))
   {  result=AmethodA(tab->caption,amd);
   }
   for(tr=tab->rows.first;tr->next && !result;tr=tr->next)
   {  for(tc=tr->cells.first;tc->next && !result;tc=tc->next)
...
{  struct Tabrow *tr;
   struct Tabcell *tc;
   struct Coldef *cd;
   while((tr=REMHEAD(&tab->rows)) || (tr=REMHEAD(&tab->tfoot)))
   {  while(tc=REMHEAD(&tr->cells))
      {  if(tc->body) Adisposeobject(tc->body);
         FREE(tc);
      }
...
static long Dispatch(struct Table *tab,struct Amessage *amsg)
{  long result=0;
   switch(amsg->method)
   {  case AOM_NEW:
      ...
      case AOM_DEINSTALL:
         break;
      default:
         AmethodasA(AOTP_ELEMENT,tab,amsg);
   }
   return result;
}
