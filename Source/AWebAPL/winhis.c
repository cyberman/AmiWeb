...
{  struct Framehis *fhis;
   if(fhis=ALLOCSTRUCT(Framehis,1,MEMF_PUBLIC|MEMF_CLEAR))
...
{  ULONG key;
   USHORT skipf=WINHF_SKIP;
   if(whis)
   {  key=whis->key;
      if(whis->wflags&WINHF_SKIP) skipf=0;
      for(whis=whis->next;whis->next;whis=whis->next)
...
{  ULONG key;
   ...
   {  key=whis->key;
      if(whis->wflags&WINHF_SKIP) skipf=0;
      for(whis=whis->prev;whis->prev;whis=whis->prev)
      {  if(whis->key==key && !(whis->wflags&skipf)) return whis;
      }
   }
   return NULL;
   ...
   {  if(wha)
      {  /* If wha is still NEW, the skip is ok but wha's NEW flag
         ...
         {  whb->wflags&=~(WINHF_SKIP|WINHF_NEW);
            key=wha->key;
            for(wha=wha->next;wha->next && wha!=whb;wha=wha->next)
            {  if(wha->key==key)
         ...
         case AOWHS_Url:
            ...
            {  fhis->url=(void *)tag->ti_Data;
               ...
               {  whis->titleurl=fhis->url;
                  if(ams->method!=AOM_NEW) chgwhw=TRUE;
               }
            }
            break;
         case AOWHS_Fragment:
...
{  struct Winhis *whis,*copy;
   ...
   {  NEWLIST(&whis->frames);
      ...
      if(copy=(struct Winhis *)GetTagData(AOWHS_Copyfrom,NULL,ams->tags))
      {  Copywinhis(whis,copy,whis->frameid?whis->frameid:NULLSTRING,
            (void *)GetTagData(AOWHS_Url,NULL,ams->tags));
      }
...
{  ObtainSemaphore(&whissema);
   while(winhis.first->next) Adisposeobject(winhis.first);
   ReleaseSemaphore(&whissema);
}

static long Dispatch(struct Winhis *whis,struct Amessage *amsg)
{  long result=0;
   switch(amsg->method)
   {  case AOM_NEW:
...
{  NEWLIST(&winhis);
   ...
   if(!Amethod(NULL,AOM_INSTALL,AOTP_WINHIS,Dispatch)) return FALSE;
   return TRUE;
}
...
{  struct Winhis *whis;
   ...
   UBYTE buf[16];
   ObtainSemaphore(&whissema);
   for(whis=winhis.first;whis->next;whis=whis->next)
   {  if(!windownr || whis->windownr==*windownr)
      {  if(!(mainline && (whis->wflags&WINHF_SKIP)))
         {  index++;
            sprintf(buf,"%d",whis->windownr);
...
{  struct Framehis *fhis;
   if(whis)
   {  printf("WINHIS %08x%s%s%s for %s\n",whis,(whis->wflags&WINHF_SKIP)?" skip":"",
         (whis->wflags&WINHF_HISTORY)?" history":"",
         (whis->wflags&WINHF_NEW)?" new":"",Frameid(whis->frameid));
      for(fhis=whis->frames.first;fhis->next;fhis=fhis->next)
      {  printf("  ID %s URL %s\n",Frameid(fhis->id),Agetattr(fhis->url,AOURL_Url));
      }
   }
}
#endif
