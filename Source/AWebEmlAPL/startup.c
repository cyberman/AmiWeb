...
/* startup.c
 *
 * General startup file for AWeb EML email plugin module
 *
...
extern __asm __saveds void Queryplugin(register __a0 struct Pluginquery *pq);

extern __asm __saveds void Filterplugin(register __a0 struct Pluginfilter *pf);

/* Function declarations for project dependent hook functions */
extern ULONG Initpluginlib(struct EmlBase *libbase);
extern void Expungepluginlib(struct EmlBase *libbase);
...
{  SysBase=sysbase;
   ...
   libseglist=seglist;
   if(!Initpluginlib((struct EmlBase *)libbase))
   {  Expungepluginlib((struct EmlBase *)libbase);
      libbase=NULL;
   }
   return libbase;
}
