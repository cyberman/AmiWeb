...
/* startup.c
 *
 * General startup file for AWeb RSS/Atom plugin module
 *
...
extern ULONG Initpluginlib(struct RssBase *libbase);
extern void Expungepluginlib(struct RssBase *libbase);
...
{  SysBase=sysbase;
   ...
   libseglist=seglist;
   if(!Initpluginlib((struct RssBase *)libbase))
   {  Expungepluginlib((struct RssBase *)libbase);
      libbase=NULL;
   }
   return libbase;
}
